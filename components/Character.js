...
export class BasicCharacterController {
	...
	_LoadModels() {
		...
		loader.setPath('./resources/entities/character/');
		loader.load('David.fbx', (fbx) => {
			fbx.scale.setScalar(0.04);  // Character Height
			fbx.traverse(c => {
				c.castShadow = true;
			});
			...
			loader.load('JumpingDown.fbx', (a) => { _OnLoad('JumpingDown', a); });
			loader.load('falling.fbx', (a) => { _OnLoad('falling', a); });
...
class WalkBackState extends State {
	...
	Enter(prevState) {
		...
		if (prevState) {
			...
			if (prevState.Name === 'idle' || prevState.Name === 'walk') {
				const ratio    = curAction.getClip().duration / prevAction.getClip().duration;
				curAction.time = prevAction.time * ratio;
			}
			else {
...
export class ThirdPersonCamera {
	...
	_CalculateIdealOffset() {
		// const idealOffset = new THREE.Vector3(-2, 7, -3);     // Character Offset Camera
		const idealOffset = new THREE.Vector3(-15, 20, -30)     // Character Offset Camera
	...
	_CalculateIdealLookat() {
		const idealLookat = new THREE.Vector3(0, 10, 30);
